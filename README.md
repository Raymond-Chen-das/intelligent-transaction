...
- **NN3.xlsx**：神經網路模型結果（預測 IC：2013/12 ~ 2025/08）
...
5. **組合構建**：
   - 13 種分組規模：10、20、50、110、120、130、150、180、200、256、400、512、1051 等分
   - 每組股票數 = 總股票數（1051）÷ 等分數
   - 做多排名最高的 g 支股票，做空排名最低的 g 支股票
   - 根據 IC 符號決定方向（IC>0: 高減低，IC<0: 低減高）
6. **績效評估**：計算每期報酬（2014/01 ~ 2025/08）並累積

### 回測設定

- **訓練起始**：1999/01
- **測試期間**：2013/12 ~ 2025/07（約 11.6 年，140 個月）
- **預測頻率**：月度（每月重新訓練和預測）
- **驗證方式**：時間序列交叉驗證（滾動視窗）
- **投資組合規模**：13 種分組方式
  - 10 等分（每組約 105 支）到 1051 等分（每組 1 支）
  - 具體分組：10、20、50、110、120、130、150、180、200、256、400、512、1051 等分
